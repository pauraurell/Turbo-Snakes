...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float maxTurbo = 1000.0f;
    public float currentTurbo;
    public float turboSpeed;

    public Slider breakSlider;
    public float maxBreak = 1000.0f;
    ...
    void Update()
    {
        ...
        if (PV.IsMine && input == true)
        {
            ...
            transform.GetChild(0).transform.Rotate(Vector3.forward * -horizontal * rotationSpeed * 4 * Time.deltaTime);
            horizontal = 0;
        ...
        if (points >= 50)
        {
            if (GameObject.Find("Canvas").transform.GetChild(5).gameObject!= null)
            {
        ...
        foreach (object o in obj)
        {
            ...
            if (g.tag == "User")
            {
                if(g.transform.GetChild(0).gameObject.GetComponent<PlayerMovement>().points >= 50 && g != transform.parent.gameObject)
                {
                    if (GameObject.Find("Canvas").transform.GetChild(6).gameObject != null)
                    {
    ...
    {
        Debug.Log("COLIDER");
        if (col.tag == "killsPlayer" && col.gameObject != tail)
        {
